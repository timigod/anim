# Map local columns with the Idris 2025 public starter

Use these files to map local columns and plan an event-based model (EBM) audit.
For setup commands, follow
[real-data integration](../../docs/handoff/real-data-integration.md#3-create-a-local-mapping-configuration).
This directory contains only public mapping guidance and project-owned
synthetic rows. It contains no Idris/LonDownS participant data, reconstructed
values, private column names, fitted output, published event order, or expected
result.
...
- The complete list of required confirmations and limits on claims is in
  [`idris-2025-public-starter-limitations.md`](../../docs/handoff/idris-2025-public-starter-limitations.md).

## Confirm the local mapping

The nine public display names are mapping targets only. Their array positions
and the aliases `event_01` through `event_09` are not the paper's event order,
an expected disease order, ground truth, or a benchmark target. Replace each
alias with a confirmed private local column and record its assay or scoring
definition, unit or range, coding, transformation, abnormal direction, and
review of whether names or values could identify participants. Every direction
remains `REQUIRES_CONFIRMATION` until that work is done.
...
- The exact residualisation formula (the adjustment for covariate effects),
  intercept, encodings, transformations, and supplied EBM values are unpublished or ambiguous.
...
- The reported `1.5 IQR` (interquartile range) sensitivity does not define the
  fitted population, variables, operation order, or action on flagged cells or
  participants.
- Separate analyses using multiple imputation by chained equations (MICE),
  principal component analysis (PCA), generalised additive models (GAM), and
  locally estimated scatterplot smoothing (LOESS) are not assumed to be EBM
  preprocessing.
...
- The paper does not identify an executable EBM package, complete model
  parameterisation, Markov chain Monte Carlo (MCMC) sampling procedure, seed,
  convergence rule, or staging formula.
- `pysaebm` is an optional example that runs an actual open-source EBM on
  synthetic data. It is not claimed to be the paper's implementation or
  scientifically interchangeable with it.
- No reference-result bundle in Anim's standard format is supplied. Similarity to a publication
...
invented solely to demonstrate the expected columns. It must never be
